...
from linebot.models import PostbackTemplateAction, PostbackEvent, ConfirmTemplate, AudioMessage, TextSendMessage, TemplateSendMessage, ButtonsTemplate, MessageTemplateAction, CarouselTemplate, CarouselColumn
...
configuration = Configuration(access_token='n0GThW+dzgtzOKw0aaiVt5mdkOVt36Ts9U6qRjSHzYU818SCSN0llBsdZ6TOFIm++AEGFZgwnYsAo0dMErjSrvI0ar+jt+F4Sx63InLgXeL7mZuTsw5lRpv7AWSuS0uN67yCJodkr9E/x7TpJgAyUgdB04t89/1O/w1cDnyilFU=')
        ...
        if is_entering:
            ...
            for para in comm_json[txt]:
                ...
                line_bot_api.reply_message_with_http_info(
                    ReplyMessageRequest(
                        reply_token=event.reply_token,
                        messages=TextSendMessage(text=para.replace('\\n', '\n\n'))
                    )
                )
        elif txt == "醫囑紀錄":
            message = TemplateSendMessage(
            ...
            line_bot_api.reply_message_with_http_info(
                    ReplyMessageRequest(
                        reply_token=event.reply_token,
                        messages=TextSendMessage(text='請開始錄音')
                    )
                )
            is_record = True
            ...
            line_bot_api.reply_message_with_http_info(
                    ReplyMessageRequest(
                        reply_token=event.reply_token,
                        messages=TextSendMessage(text='現在起您可以開始編輯文字')
                    )
                )
            is_entering = True
        elif txt == "是，紀錄在今天的醫囑":
            ...
            line_bot_api.reply_message_with_http_info(
                    ReplyMessageRequest(
                        reply_token=event.reply_token,
                        messages=TextSendMessage(text="好的，已經幫您紀錄於本日醫囑\n"+date)
                    )
                )
        elif txt == "否，不需要手動編輯":
            message = TemplateSendMessage(
            ...
            line_bot_api.reply_message_with_http_info(
                    ReplyMessageRequest(
                        reply_token=event.reply_token,
                        messages=TextSendMessage(text=strn)
                    )
                )
        elif txt == '營養諮詢':
            message = TemplateSendMessage( alt_text='轉盤樣板',
    ...
    with ApiClient(configuration) as api_client:
        ...
        if backdata.get('action') == 'reserve':
            ...
            line_bot_api.reply_message_with_http_info(
                        ReplyMessageRequest(
                            reply_token=event.reply_token,
                            messages=TextSendMessage(text='預約成功')
                        )
                    )
